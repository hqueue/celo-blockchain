...
// caching it if found.
func (bc *BlockChain) GetAccumulatedUptime() []istanbul.Uptime {
	// Short circuit if the uptime's already in the cache, retrieve otherwise
	if cached, ok := bc.uptimeCache.Get("uptime"); ok {
		return cached.([]istanbul.Uptime)
	}
	uptime := rawdb.ReadAccumulatedUptime(bc.db)
	if uptime == nil {
		return nil
	}
	// Cache the found body for next time and return
	bc.uptimeCache.Add("uptime", uptime)
	return uptime
}

// WriteAccumulatedUptime stores the accumulated uptime for an epoch into the database, also caching it
// along the way.
func (bc *BlockChain) WriteAccumulatedUptime(uptime []istanbul.Uptime) error {
	rawdb.WriteAccumulatedUptime(bc.db, uptime)
	bc.uptimeCache.Add("uptime", uptime)
	return nil
}
...
func updateUptime(uptime []istanbul.Uptime, blockNumber uint64, bitmap *big.Int, window uint64) []istanbul.Uptime {
	...
	for i := 0; i < len(uptime); i++ {
		if bitmap.Bit(i) == 1 {
			...
			// even though they did not sign, if their last signed block is within the window they should still be considered alive (do not reward though blocks whcih were before the first window)
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	if bc.engine.Protocol().Name == "istanbul" {
		...
		// write the new score
		bc.WriteAccumulatedUptime(uptime)
	}
