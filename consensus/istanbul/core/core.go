...
func (c *core) newLogger(ctx ...interface{}) log.Logger {
	...
	return logger.New("cur_seq", seq, "cur_round", round, "state", state)
}
...
func (c *core) waitForDesiredRound(r *big.Int) {
	...
	c.valSet.CalcProposer(lastAuthor, r.Uint64())
	...
	if c.current != nil {
		if roundChange {
			...
			if err != nil && c.current.Proposal() != nil && c.current.Proposal().Hash() == lastSubject.Digest && c.current.Round().Cmp(lastSubject.View.Round) == 0 {
				...
				c.current = newRoundState(view, validatorSet, nil, istanbul.EmptyPreparedCertificate(), newMessageSet(c.backend.ParentBlockValidators(lastBlock)))
			}
		}
	} else {
